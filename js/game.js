(function () {
  ...
  const MINIMAP_MIN_SPAN = 600;
  const MINIMAP_PADDING = 80;
  const BEST_TIME_KEY = "survivor_best_time";
  const BEST_KILLS_KEY = "survivor_best_kills";
  ...
  const PLAYER_SPRITE = {
    ...
    scale: 2,
    animFps: 8,
    // 1열=아래, 2열=위, 3열=왼쪽, 4열=오른쪽 (GAME_PLAN 4.1)
  ...
  const levelupLevelText = document.getElementById("levelup-level-text");
  ...
  const bestRecordMenu = document.getElementById("best-record-menu");
  const bestRecordGameover = document.getElementById("best-record-gameover");
  ...
  window.addEventListener("keyup", (e) => {
    if (state === GameState.PAUSED || state === GameState.LEVEL_UP) return;
    keys[e.code] = false;
  });
  ...
  function updateSoundButtons() {
    ...
    menuSoundBtn.textContent = soundEnabled ? "🔊 사운드" : "🔇 사운드";
    menuSoundBtn.classList.toggle("muted", !soundEnabled);
  }
  ...
  function worldToScreen(wx, wy) {
    return {
      x: wx - camera.x + CANVAS_W / 2,
      y: wy - camera.y + CANVAS_H / 2,
    };
  }

  function isOnScreen(wx, wy, margin) {
    const m = margin || 0;
    const sx = wx - camera.x + CANVAS_W / 2;
    const sy = wy - camera.y + CANVAS_H / 2;
    return sx > -m && sx < CANVAS_W + m && sy > -m && sy < CANVAS_H + m;
  }

  function getCooldownMult(p) {
  ...
  function pickRandomEnemies(list, count) {
    const copy = [...list];
  ...
  function createScythe(level) {
    return {
      ...
      draw() {
        for (let b = 0; b < this.bladeCount; b++) {
          const a = this.angle + (b / this.bladeCount) * Math.PI * 2;
          const bx = player.x + Math.cos(a) * this.orbitRadius;
          const by = player.y + Math.sin(a) * this.orbitRadius;
    ...
    return {
      ...
      update(dt, p, enemyList) {
        ...
        for (const t of targets) {
          ...
          lightningFlashes.push({ x: t.x, y: t.y, life: 0.3, maxLife: 0.3 });
  ...
  function updatePlayer(dt) {
    ...
    for (const weapon of player.weapons) {
      weapon.update(dt, player, enemies, projectiles);
    }
  }

  // --- Enemy ---
  function createEnemy(x, y, elapsed) {
  ...
  function killEnemy(enemy) {
    ...
    pickups.push(createPickup(enemy.x, enemy.y, enemy.xpValue));
  ...
  function spawnEnemy() {
    ...
    enemies.push(createEnemy(x, y, survivalTime));
  }
  ...
  function updateEnemies(dt) {
    for (let i = enemies.length - 1; i >= 0; i--) {
      ...
      if (dir.x !== 0 || dir.y !== 0) {
        e.facing = getPlayerFacing(dir.x, dir.y, e.facing);
        e.animTimer += dt;
      ...
      if (circleOverlap(player.x, player.y, player.radius, e.x, e.y, e.radius)) {
        if (player.invulnTimer <= 0 && e.contactCooldown <= 0) {
          player.hp -= e.damage;
  ...
  function updatePickups(dt) {
    for (let i = pickups.length - 1; i >= 0; i--) {
      ...
      if (d < player.pickupRadius) {
        ...
        const speed = gem.magnetSpeed + (player.pickupRadius - d) * 2;
        gem.x += dir.x * speed * dt;
        gem.y += dir.y * speed * dt;
      }

      if (circleOverlap(player.x, player.y, player.radius, gem.x, gem.y, gem.radius)) {
  ...
  function addXp(amount) {
    ...
    if (player.xp >= player.xpToNextLevel) {
      ...
      player.level++;
      player.xpToNextLevel = xpToNext(player.level);
      showLevelUp();
    }
  }
  ...
  function buildUpgradePool() {
    ...
    for (const id of Object.keys(WEAPON_FACTORIES)) {
      if (!ownedIds.has(id) && player.weapons.length < MAX_WEAPONS) {
        pool.push({
          type: "weapon_new",
          weaponId: id,
  ...
  function applyUpgrade(option) {
    if (option.type === "weapon_new") {
      player.weapons.push(WEAPON_FACTORIES[option.weaponId](1));
    } else if (option.type === "weapon_up") {
      const idx = player.weapons.findIndex((w) => w.id === option.weaponId);
      if (idx !== -1) {
        const newLevel = player.weapons[idx].level + 1;
        player.weapons[idx] = WEAPON_FACTORIES[option.weaponId](newLevel);
      }
    } else if (option.type === "passive") {
      PASSIVE_DEFS[option.passiveId].apply(player);
    }
  }

  function renderUpgradeCards() {
    upgradeCardsEl.innerHTML = "";
    currentUpgrades.forEach((opt, i) => {
  ...
  function showLevelUp() {
    ...
    levelupLevelText.textContent = `Lv. ${player.level}`;
    renderUpgradeCards();
  ...
  function selectUpgrade(index) {
    ...
    if (!currentUpgrades[index]) return;
  ...
  function updateSpawning(dt) {
    const diff = getDifficulty(survivalTime);
    const rate = 1.2 * (1 + survivalTime * 0.018) * diff.spawnMult;
    spawnTimer += dt;
    const interval = 1 / rate;
    while (spawnTimer >= interval) {
      spawnTimer -= interval;
      spawnEnemy();
      if (survivalTime > 60 && survivalTime < 180 && Math.random() < 0.22) {
        spawnEnemy();
      }
      if (survivalTime >= 180 && Math.random() < 0.12) {
        spawnEnemy();
      }
    }
  }
  ...
  function drawBackground() {
    ...
    const startX = Math.floor((camera.x - CANVAS_W / 2) / tileW) * tileW;
    const startY = Math.floor((camera.y - CANVAS_H / 2) / tileH) * tileH;
    const endX = camera.x + CANVAS_W / 2 + tileW;
    const endY = camera.y + CANVAS_H / 2 + tileH;
  ...
  function drawPickups() {
    for (const gem of pickups) {
      ...
      const pulse = 1 + Math.sin(survivalTime * 8 + gem.x) * 0.15;
  ...
  function drawPlayer() {
    const blink = player.invulnTimer > 0 && Math.floor(player.invulnTimer * 10) % 2 === 0;
    const s = worldToScreen(player.x, player.y);

    if (PLAYER_SPRITE.loaded && PLAYER_SPRITE.image) {
      const col = PLAYER_SPRITE.dirCol[player.facing];
      const row = player.isMoving ? player.animFrame : 0;
      ...
      drawCircle(player.x, player.y, player.radius, COLORS.player, blink ? 0.4 : 1);
    }

    ctx.strokeStyle = "rgba(0, 217, 255, 0.15)";
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.arc(s.x, s.y, player.pickupRadius, 0, Math.PI * 2);
    ctx.stroke();
  }
  ...
  function drawMinimap() {
    ...
    const pp = worldToMinimap(player.x, player.y);
  ...
  function updateHud() {
    ...
    levelDisplay.textContent = String(player.level);

    timeDisplay.textContent = formatTime(survivalTime);
    killDisplay.textContent = `처치: ${killCount}`;
  }
  ...
  function pauseGame() {
    ...
    clearKeys();
    pauseTime.textContent = formatTime(survivalTime);
    pauseKills.textContent = String(killCount);
    pauseLevel.textContent = String(player.level);
  ...
  function endGame() {
    ...
    const best = getBestRecord();
    const isNewRecord = survivalTime > best.time;
    if (isNewRecord) {
      localStorage.setItem(BEST_TIME_KEY, String(survivalTime));
      localStorage.setItem(BEST_KILLS_KEY, String(killCount));
    }

    gameoverTime.textContent = formatTime(survivalTime);
    gameoverKills.textContent = String(killCount);
    gameoverLevel.textContent = String(player.level);
